  ...
  useEffect(() => {
    ...
    function openMenu() {
      if (!open) {
        ...
        gsap.to('.menu', { color: "var(--color-black)", delay: 0.5, duration: 0.5 });
      } else {
        open = false;
        gsap.to('.menu-open', { opacity: 0, delay: 1, duration: 0.5 });
        gsap.to('.menu', { color: "var(--color-white)", delay: 0.5, duration: 0.5 });
      }
    }
    ...
    return () => {
      ...
      if (linkLogo) linkLogo.removeEventListener('click', () => location.reload());
    };
